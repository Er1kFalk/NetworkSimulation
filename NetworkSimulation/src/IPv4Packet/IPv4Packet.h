...
public:
    ...
    TrafficClassInterface* get_type_of_service() override {return this->type_of_service;}
    ...
    ProtocolTypeInterface* get_protocol() override {return this->protocol;}
    std::array<unsigned char, 2> get_header_checksum() override {return this->header_checksum;}
    IPv4AddressInterface* get_source() override {return this->source;}
    IPv4AddressInterface* get_destination() override {return this->destination;}

    IPv4Packet(TrafficClassInterface *t, int identification_id, bool df, bool mf, unsigned int offset, unsigned char time_to_live, ProtocolTypeInterface *p, IPv4AddressInterface *source, IPv4AddressInterface *destination, CommunicationProtocol *payload) : IPv4PacketInterface() {
        set_version();
        set_ihl();
